...
class ToursService {
  ...
  // new tour
  tourNew = async (dataTour: any): Promise<TourData> => {
    try {
  ...
  // update tour by id
  updateTourId = async (dataTour: any): Promise<TourData> => {
    try {
      let tourData: TourData;
          ...
          // Cập nhật dữ liệu
          {
            $set: {
              ...
              estimatedTime: (tour.estimatedTime = dataTour?.estimatedTime
                ...
                : tourHolder.estimatedTime
                ? new Date(tourHolder.estimatedTime)
                : new Date()),
              closeOrderTime: (tour.closeOrderTime = dataTour?.closeOrderTime
                ? new Date(dataTour.closeOrderTime)
                : tourHolder.closeOrderTime
                ? new Date(tourHolder.closeOrderTime)
                : new Date()),
              limit: dataTour?.limit || tourHolder.buySlot,
              images: generateUrlImage(dataTour?.images) ||
                tourHolder.images || [
                  {
                    urlImage: 'https://achautravel.com/upload/images/1689131743.jpeg'
                  },
      ...
      if (type === 'add' && Array.isArray(imageUrls)) {
        ...
        if (typeof imageUrls === 'string') {
          ...
          if (tourImage && Array.isArray(tourImage) && updateList.length > 0) {
            tourImage = tourImage.filter(
              (tourItem) => !updateList.some((updateItem) => updateItem.urlImage === tourItem.urlImage)
            );
          }

          imageUrls.split(',').map(async (url: string) => await instanceStorageFireball.deleteImage(url.trim()));
  ...
  filterTourCustomer = async (
    ...
    try {
      ...
      if (keyword) {
        ...
        query.$or = [{ name: { $regex: regex } }, { description: { $regex: regex } }, { address: { $regex: regex } }];
      }
        ...
        query.$expr = {
          $or: priceRanges
            .map((range: string) => {
              ...
              if (from && to) {
                return {
                  $and: [{ $gte: [{ $toDouble: '$priceAdult' }, from] }, { $lte: [{ $toDouble: '$priceAdult' }, to] }]
                };
              } else if (from) {
  ...
  // cancel booking with id tour

  CancelTourComing = async (id: string): Promise<TourData> => {
    try {
      let tourData: TourData;
      ...
      if (tour) {
        ...
        tour.delFlg = 1;
        await managerTour.getMongoRepository(Tour).save(tour);
        const cancel = await bookingService.cancelBooking(id);
        if (cancel) {
          //sent mail
          tourData = {
            status: 200,
            errCode: 200,
            errMessage: `Tour cancel successfully.`
          };
        } else {
          tourData = {
            status: 400,
  ...
  getListTourComing = async (perPage: number | null = null, currentPage: number = 1): Promise<TourData> => {
    try {
      let tourData: TourData;
      const options: FindOptions = {};
      if (perPage) {

...
class CommonController {
    ...
    handleRequestOTPExist = async(req: Request, res: Response, next: NextFunction)=>{
        try{
            logger.info(`[P]::request Otp exist:: email : `);
            const { email } = req.body;
            if(!email ){
                return res.status(400).json({
                    errCode: 400,
            ...
            const sentOTP = await commonService.requestOTPExist(email)
            return res.status(sentOTP.status).json({
                errCode: sentOTP.errCode,
                message: sentOTP.errMessage,
                ttl: sentOTP.ttl
